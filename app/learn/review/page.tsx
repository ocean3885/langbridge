...
export default async function ReviewPage() {
  ...
  const recommendedHref = recommendation ? reviewHref(recommendation, otherCount) : '/bundles';
  const reviewTypes = [
...
function getRecommendation(summary: Awaited<ReturnType<typeof getReviewNeededSummary>>): 'words' | 'sentences' | null {
  if (!summary.availableTotal) return null;
  if (summary.availableWords !== summary.availableSentences) return summary.availableWords > summary.availableSentences ? 'words' : 'sentences';
...
function reviewHref(type: 'words' | 'sentences', otherCount: number) { const base = `/learn/review/${type}`; const other = type === 'words' ? 'sentences' : 'words'; return otherCount > 0 ? `${base}?nextTo=/learn/review/${other}` : base; }
