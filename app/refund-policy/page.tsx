...
const content = {
  ko: {
    ...
    sections: [
      ['1. 환불 요청', '결제일, 계정 이메일, Paddle 거래 또는 영수증 정보를 포함하여 ocean3885@gmail.com으로 요청해 주세요. 확인을 위해 추가 정보를 요청할 수 있습니다. 구독 해지는 프로필의 구독 관리 기능에서 별도로 진행할 수 있습니다.'],
      ...
      ['6. 자동 갱신 및 해지', '프리미엄 구독은 회원이 해지할 때까지 선택한 월간 또는 연간 주기로 자동 갱신됩니다. 구독 해지는 장래의 갱신을 중단하는 절차이며 이미 완료된 결제를 자동으로 환불하지 않습니다. 별도 안내가 없는 한 해지 후에도 현재 결제 기간 종료일까지 이용할 수 있습니다.'],
      ['7. 갱신 결제 환불', '갱신 결제에 대한 환불 가능 여부는 요청 시점, 서비스 이용 이력, 결제수단 및 관계 법령을 기준으로 판단합니다. 다음 결제를 원하지 않는 경우 갱신일 전에 구독을 해지해 주세요.'],
    ],
    termsLink: '이용약관 확인',
  },
  en: {
    ...
    sections: [
      ['1. Requesting a refund', 'Email ocean3885@gmail.com with the payment date, account email, and Paddle transaction or receipt details. Additional information may be requested for verification. Subscription cancellation is available separately through billing management in your profile.'],
      ...
      ['6. Automatic renewal and cancellation', 'Premium renews monthly or yearly until canceled. Cancellation stops future renewals but does not automatically refund a completed charge. Unless otherwise stated, access continues through the end of the current billing period after cancellation.'],
      ['7. Renewal charge refunds', 'Eligibility for a refund of a renewal charge depends on the request date, service usage, payment method, and applicable law. Cancel before the renewal date if you do not want the next charge.'],
    ],
