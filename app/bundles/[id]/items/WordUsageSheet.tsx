...
export default function WordUsageSheet({
  words,
  details,
  language,
  copy,
  ...
  const selectedMapping = selectedWordId ? words.find((word) => word.word_id === selectedWordId) : null;
  ...
  useEffect(() => {
    ...
    function handleKeyDown(event: KeyboardEvent) {
      if (event.key === 'Escape') setSelectedWordId(null);
    }
  ...
  return (
    <>
      <div className="mt-3 flex flex-wrap gap-2" aria-label={copy.words}>
        {words.map((word) => {
