    ...
    <section>
      ...
      <div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-6">
        {categories.map((category, index) => {
          ...
          const content = (
            <>
              <div className={`flex h-16 w-16 shrink-0 items-center justify-center rounded-full sm:mx-auto sm:h-20 sm:w-20 ${style.color}`}>
          ...
          if (count === 0) {
            return (
              <div
                ...
                className="flex min-h-0 items-center gap-4 rounded-xl border border-zinc-200 bg-white p-4 opacity-70 shadow-sm sm:min-h-64 sm:flex-col sm:items-stretch sm:gap-0 sm:p-5"
              >
                {content}
              </div>
            );
