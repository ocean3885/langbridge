      ...
      <div className="min-h-screen bg-gray-50 dark:bg-background md:ml-64 p-8">
        <div className="max-w-6xl mx-auto">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 dark:text-white">채널 관리</h1>
              <p className="text-gray-600 dark:text-gray-400 mt-2">언어별 학습 채널을 등록/관리합니다.</p>
            </div>
            <Link
          ...
          {channels.length === 0 ? (
            <div className="text-center py-12 text-gray-500 dark:text-gray-400">등록된 채널이 없습니다. 새 채널을 추가해보세요.</div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {channels.map((ch) => {
                ...
                return (
                  <div key={ch.id} className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg hover:shadow-lg transition-shadow p-5">
                    {/* 헤더: 썸네일 + 정보 */}
                    <div className="flex items-start gap-4 mb-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="text-lg font-semibold mb-1 truncate text-gray-900 dark:text-gray-100">{ch.channel_name}</h3>
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-sm font-medium text-gray-700 dark:text-gray-300">언어:</span>
                          <span className="text-sm text-gray-600 dark:text-gray-400">{languageName}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-medium text-gray-700 dark:text-gray-300">영상:</span>
                          <span className="text-sm text-blue-600 dark:text-blue-400 font-semibold">{videoCount}개</span>
                        </div>
                      </div>
                    ...
                    <div className="space-y-2">
                      ...
                      {ch.channel_description && (
                        <p className="text-sm text-gray-600 dark:text-gray-400 line-clamp-2">{ch.channel_description}</p>
                      )}
                    </div>
                  </div>
                );
