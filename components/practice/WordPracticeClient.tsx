...
interface WordsReviewClientProps {
  ...
  bundleSession?: {
    bundleId: string;
    title: string;
    mode: 'word_quiz' | 'word_flashcards';
    isLoggedIn: boolean;
    bundleItemIds: Record<number, string>;
  };
  optionItems?: ReviewWordItem[];
}
...
export default function WordPracticeClient({ initialItems, wordUsageDetails, availableReviewCount, language, sessionKind = 'review', returnTo, nextTo, bundleSession, optionItems }: WordsReviewClientProps) {
  const t = copy[language];
  const isLearning = sessionKind === 'learning';
  const returnHref = bundleSession ? `/bundles/${bundleSession.bundleId}/${bundleSession.mode === 'word_quiz' ? 'word-quiz' : 'word-flashcards'}` : isLearning ? '/learn/progress/words' : returnTo || '/learn/review';
  const returnLabel = bundleSession ? (language === 'ko' ? '문제 다시 선택' : 'Change Set') : isLearning || returnTo ? t.backToLearn : t.backToReview;
  ...
  useEffect(() => {
    ...
    if (currentItemMode === 'quiz') {
      const distractors = buildWordMeaningDistractors(currentItem, optionItems || initialItems, isEnglish);
  ...
  useEffect(() => {
    ...
    void fetch('/api/bundle-progress', {
      ...
      body: JSON.stringify({
        bundle_id: bundleSession.bundleId,
        current_bundle_item_id: bundleSession.bundleItemIds[currentItem.id],
        current_word_id: currentItem.id,
        current_practice_mode: bundleSession.mode,
      }),
    });
  }, [bundleSession, currentItem, step]);

  const recordResult = (wordId: number, isCorrect: boolean, mode: 'quiz' | 'spelling' | 'flashcards') => {
    if (!bundleSession) return recordWordResult(wordId, isCorrect, mode);
    if (!bundleSession.isLoggedIn) return;
    void fetch('/api/bundle-progress', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        bundle_id: bundleSession.bundleId,
        bundle_item_id: bundleSession.bundleItemIds[wordId],
        word_id: wordId,
        practice_mode: bundleSession.mode,
  ...
  const checkSpelling = () => {
    ...
    if (!isRetryAttempt) {
      recordResult(currentItem.id, answerIsCorrect, 'spelling');
    }
  ...
  const selectOption = (option: string) => {
    ...
    recordResult(currentItem.id, answerIsCorrect, 'quiz');
  ...
  const handleFlashcardEvaluate = (known: boolean) => {
    ...
    recordResult(currentItem.id, known, 'flashcards');
  ...
  if (initialItems.length === 0) {
    return (
      <div className="mx-auto flex min-h-[70vh] max-w-xl flex-col items-center justify-center gap-6 text-center px-4">
        ...
        <h1 className={`${headingClass} text-3xl dark:text-zinc-100`}>{bundleSession ? (language === 'ko' ? '연습할 단어가 없습니다.' : 'No words to practice.') : isLearning ? t.learningEmptyTitle : t.emptyTitle}</h1>
        <p className="text-zinc-500 dark:text-zinc-400 max-w-md leading-relaxed">{bundleSession ? bundleSession.title : isLearning ? t.learningEmptyDesc : t.emptyDesc}</p>
  ...
  if (step === 'finished') {
    return (
      <div className="mx-auto flex min-h-[70vh] max-w-xl flex-col items-center justify-center gap-6 text-center px-4">
        <h1 className={`${headingClass} text-4xl dark:text-zinc-100`}>{bundleSession ? (language === 'ko' ? '학습 완료' : 'Practice complete') : isLearning ? t.learningDoneTitle : t.doneTitle}</h1>
        <div className="my-6">
        ...
        <div className="flex flex-wrap justify-center gap-3 mt-4">
          ...
          <button onClick={restart} className="inline-flex items-center gap-2 rounded-lg border border-zinc-200 bg-white px-6 py-3 text-sm font-bold text-zinc-700 transition hover:bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-200 dark:hover:bg-zinc-800">
            ...
            {bundleSession ? t.retryBtn : isLearning ? t.learningRestartBtn : t.restartBtn}
          </button>
        </div>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8">
      ...
      <header className="mb-6 flex items-center justify-between">
        {bundleSession ? <Link href={returnHref} aria-label={returnLabel} className="rounded-full p-2 text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800"><ArrowLeft className="h-5 w-5" /></Link> : <button onClick={restart} className="rounded-full p-2 text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800"><ArrowLeft className="h-5 w-5" /></button>}
      ...
      {bundleSession && <p className="mb-4 text-sm font-semibold">{bundleSession.title}</p>}
...
function recordWordResult(wordId: number, isCorrect: boolean, mode: 'quiz' | 'spelling' | 'flashcards') {
  void fetch('/api/word-progress', {
    ...
    body: JSON.stringify({
      word_id: wordId,
      is_correct: isCorrect,
      practice_mode: mode === 'quiz' ? 'word_quiz' : mode === 'flashcards' ? 'word_flashcards' : mode,
    }),
  });
}
